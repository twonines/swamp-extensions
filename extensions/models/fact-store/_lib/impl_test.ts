// ABOUTME: Unit tests for @twonines/fact-store's _lib/impl.ts.
...
import type { ExportConstraint, ExportFact } from "./impl.ts";
...
const CONSTRAINT_SECRET: ExportConstraint = {
  id: "c-secret",
  kind: "security_boundary",
  scope: "global",
...
Deno.test("runExport - embeds and writes both the state and the raw bytes", async () => {
  ...
  try {
    const { state, bytes } = await runExport(
      [FACT_BACKUP, FACT_OWNER],
      [CONSTRAINT_SECRET],
      baseGlobalArgs({ outputPath: tmpPath }),
      testLogger(),
    );

    assertEquals(state.factCount, 2);
    assertEquals(state.constraintCount, 1);
    assertEquals(state.outputBytes, bytes.byteLength);
...
Deno.test("runExport - empty corpus skips the embeddings call entirely", async () => {
  ...
  try {
    ...
    assertEquals(state.factCount, 0);
...
Deno.test("runExport - throws when embedUrl/embedToken are missing", async () => {
  const tmpPath = await Deno.makeTempFile({ suffix: ".db" });
  try {
    let threw = false;
    try {
      await runExport([], [], { outputPath: tmpPath }, testLogger());
    } catch {
      threw = true;
    }
...
Deno.test("runSearch - throws when embedUrl/embedToken are missing", async () => {
  let threw = false;
  try {
    await runSearch(
      new Uint8Array(),
